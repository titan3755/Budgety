...
import { IconUser, IconLock, IconLogout } from '@tabler/icons'
...
{/* 
          ...
          <div className="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
            <div className="flex-shrink-0 flex items-center">
              ...
              <h1 className="text-indigo-500 font-bold text-2xl ml-[5px] mr-2 mb-[2px] font-alata">Budgety</h1>
            </div>
          ...
          <div className="absolute inset-y-0 right-0 items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0 hidden sm:block">
            {
              session ? <UserMenuDropdown username={session.user.name} avatar={session.user.image} email={session.user.email} /> : <Button variant="outline" color="indigo" className="mx-1"onClick={() => {signIn()}}>Login</Button>
            }
      ...
      <div className={mobileNavState ? "visible" : "hidden"} id="mobile-menu">
        <div className="px-2 pt-2 pb-3 space-y-1">
          ...
          <div className="flex flex-row align-middle justify-center items-center">
            {
              session ? <UserMenuDropdown username={session.user.name} avatar={session.user.image} email={session.user.email} /> :  <Button variant="outline" color="indigo" className="mx-1" fullWidth onClick={() => {signIn()}}>Login</Button>
            }
          </div>
        </div>
      </div>
...
function UserMenuDropdown({username, avatar, email}) {
  return (
    <Menu shadow="md" width={200}>
      <Menu.Target>
        {
          avatar ? <Avatar src={avatar} className="hover:brightness-75" alt="user avatar" radius="xl" /> : <Avatar src={gravatar.url(email)} className="hover:brightness-75" alt="user avatar" radius="xl" />
        }
